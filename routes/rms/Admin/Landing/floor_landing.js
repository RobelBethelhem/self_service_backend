...
router.get('/get_floor', auth, roleCheck(["admin", "site_manager"]), async (req, res, next) => {
  console.log("I am in the landing floor");
  ...
  try {
    ...
    async function getFloorWithBuildingSiteAndPlaceData() {
      try {
        if(userRole[0].includes('admin')){
           floors = await Floor.find().lean();
        }
        else if(userRole[0].includes('site_manager')){
          floors = await Floor.find({site_id: user.site_id}).lean();
         }
        

        const newObj = await Promise.all(floors.map(async (item) =>{
          try{
            if (item.building_id) {
    ...
    // Usage
    try {
      const result = await getFloorWithBuildingSiteAndPlaceData();
      res.json({
        data: result,
