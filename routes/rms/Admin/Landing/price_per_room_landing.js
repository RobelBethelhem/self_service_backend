...
router.get('/get_price_per_room_data', auth, roleCheck(["admin"]), async (req, res, next) => {
    ...
    try {
      ...
      async function getRoomthenFloorthenBuildingthenSitethenPlacedata() {
        try {
          const Price_Per_Rooms = await Price_Per_Room.find().lean();
          const newObj = await Promise.all(Price_Per_Rooms.map(async (item) => {
            try {
                if(item.room_id){
                    const {Room_name, sqrt, room_description, floor_id} = await Room.findById(item.room_id).lean();

                if(floor_id){
                       
                    const {floor, floor_description, building_id} = await Floor.findById(floor_id).lean();
                    const ppf = await Price_Per_Floor.findOne({floor_id: floor_id})
                    let min_selling_price_per_cube_for_floor,max_selling_price_per_cube_for_floor;
                    if(ppf){
                        ({ min_selling_price_per_cube_for_floor,  max_selling_price_per_cube_for_floor} = ppf)
                    }
              if (building_id) {
                const building = await Building.findById(building_id).lean();
                if (building) {
                    let min_selling_price_per_cube_for_building;
                  let max_selling_price_per_cube_for_building;

                    const ppb = await Price_Per_Building.findOne({building_id: building_id});
                    if(ppb){
                        ({ min_selling_price_per_cube_for_building,  max_selling_price_per_cube_for_building} = ppb)
                    }
                  ...
                  if (!hasScriptProperty) {
                    return {
                      ...
                      sqrt,
                      room_description,
                      min_selling_price_per_cube_for_floor,
                      max_selling_price_per_cube_for_floor,
                      min_selling_price_per_cube_for_building,
                      max_selling_price_per_cube_for_building,
                      building_name,
                      total_floor,
                      special_feature,
      ...
      // Usage
      try {
        const result = await getRoomthenFloorthenBuildingthenSitethenPlacedata();
        res.json({
          data: result,
