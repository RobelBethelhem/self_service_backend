...
router.get('/get_price_per_floor_data', auth, roleCheck(["admin"]), async (req, res, next) => {
   console.log("In ppf")
    ...
    try {
      ...
      async function getFloorthenBuildingthenSitethenPlacedata() {
        try {
          const Price_Per_Floors = await Price_Per_Floor.find().lean();
          const newObj = await Promise.all(Price_Per_Floors.map(async (item) => {
            try {
                if(item.floor_id){
                       
                    const {floor, floor_description, building_id} = await Floor.findById(item.floor_id).lean();
              if (building_id) {
      ...
      // Usage
      try {
        const result = await getFloorthenBuildingthenSitethenPlacedata();
        res.json({
          data: result,
